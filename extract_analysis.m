% EXTRACT Results: Baseline vs Injection — batch or single experiment
% Crystal-skull / mesoscope whole dorsal cortex (EXTRACT post-processing)
%
% Batch: under root_hazon, processes each subfolder that contains both
%   Baseline/<bl_subfolder> and Injection/<inj_subfolder>.
% Outputs: <out_root>/<experiment_id>/fig*.png and summary_stats.txt
%
% See README.md, docs/DATASET_STRUCTURE.md, and docs/METHODS.md

clear; close all; clc;
t_batch = tic;

%% ========== CONFIG ==========
batch_mode       = true;   %% false = only exp_list_manual{1}
root_hazon       = '\\stg-tnr50a1.stanford.edu\crystal_skull\omer_hazon';
bl_subfolder     = 'mirror2path';   %% subfolder under Baseline/ (e.g. mirror1path)
inj_subfolder    = 'mirror2path';   %% subfolder under Injection/

here = fileparts(mfilename('fullpath'));
if isempty(here), here = pwd; end
out_root         = fullfile(here, 'extract_figures');  %% one subfolder per experiment id

exp_list_manual  = {'CS1014-1b'};
exp_list_filter  = {};   %% e.g. {'CS1014-1b','CS0204-1b'} to restrict batch
...
if batch_mode
    ...
    for k = 1:numel(d)
        ...
        bl_chk = fullfile(root_hazon, d(k).name, 'Baseline', bl_subfolder);
        ij_chk = fullfile(root_hazon, d(k).name, 'Injection', inj_subfolder);
        if exist(bl_chk, 'dir') ~= 7 || exist(ij_chk, 'dir') ~= 7
            continue;
        end
        exp_list{end+1} = d(k).name; %#ok<AGROW>
    end
    exp_list = sort(exp_list);
else
    exp_list = exp_list_manual;
end

if ~isempty(exp_list_filter)
    exp_list = exp_list(ismember(exp_list, exp_list_filter));
end

if isempty(exp_list)
    error('extract_analysis:NoExperiments', ...
        'No experiments to process. Check root_hazon, bl_subfolder, inj_subfolder, and folder layout.');
end

fprintf('Batch: %d experiment(s): %s\n', numel(exp_list), strjoin(exp_list, ', '));

for iExp = 1:numel(exp_list)
    exp_name = exp_list{iExp};
    base_dir = fullfile(root_hazon, exp_name);
    bl_path  = fullfile(base_dir, 'Baseline', bl_subfolder);
    inj_path = fullfile(base_dir, 'Injection', inj_subfolder);
    out_dir  = fullfile(out_root, exp_name);
    if ~exist(out_dir, 'dir'), mkdir(out_dir); end

    fprintf('\n%s\n', repmat('=', 1, 72));
    fprintf('Experiment %d/%d: %s\n', iExp, numel(exp_list), exp_name);
    fprintf('%s\n', repmat('=', 1, 72));

    req_bl = { fullfile(bl_path,'M_moco_frr.mat'), fullfile(bl_path,'M_moco_ds_ext.mat'), ...
        fullfile(bl_path,'M_summary.mat'), fullfile(bl_path,'cell_map.png') };
    req_ij = { fullfile(inj_path,'M_moco_frr.mat'), fullfile(inj_path,'M_moco_ds_ext.mat'), ...
        fullfile(inj_path,'M_summary.mat'), fullfile(inj_path,'cell_map.png') };
    miss = '';
    for ri = 1:numel(req_bl)
        if exist(req_bl{ri}, 'file') ~= 2
            miss = [miss sprintf('  missing %s\n', req_bl{ri})]; %#ok<AGROW>
        end
    end
    for ri = 1:numel(req_ij)
        if exist(req_ij{ri}, 'file') ~= 2
            miss = [miss sprintf('  missing %s\n', req_ij{ri})]; %#ok<AGROW>
        end
    end
    if ~isempty(miss)
        fprintf('SKIP (incomplete files):\n%s', miss);
        continue;
    end
    ...
    % Wilcoxon: per-1s-bin fraction active, BL vs Inj (synchrony panel title + summary_stats)
    ...
    sgtitle(sprintf('%s — Field of View & Detected Neurons', exp_name),'FontSize',15,'FontWeight','bold');
    ...
    sgtitle(sprintf('%s — Population-Level Activity: Baseline vs Injection', exp_name),'FontSize',15,'FontWeight','bold');
    ...
    L = {};
    L{end+1} = sprintf('%s | EXTRACT Results: Baseline vs Injection', exp_name);
    L{end+1} = repmat('=', 1, 90);
...
elapsed = toc(t_batch);
fprintf('\n========== Batch finished: %d experiment(s), total %.1f s (%.1f min) ==========\n', ...
    numel(exp_list), elapsed, elapsed/60);
