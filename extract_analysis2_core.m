function extract_analysis2_core(bl_path, inj_path, out_dir, run_id, params)
%EXTRACT_ANALYSIS2_CORE  Baseline vs Injection population analysis (single run).
...
col_bl  = getp(params, 'col_bl',  [0.13 0.59 0.95]);
col_inj = getp(params, 'col_inj', [1.00 0.34 0.13]);
z_thresh = getp(params, 'z_thresh', 2);
n_boot   = getp(params, 'n_boot', 1000);
...
sgtitle(sprintf('%s — Field of View & Detected Neurons', run_id),'FontSize',15,'FontWeight','bold');
...
fprintf('Run elapsed: %.1f s (%.1f min)\n', elapsed, elapsed/60);
